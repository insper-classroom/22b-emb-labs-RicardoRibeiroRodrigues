...
void send_get(uint8_t *g_sendBuffer, char *path);
  ...
  while(1){
    ...
    switch(state){
      ...
      if(xQueueReceive(xQueueMsg, &p_recvMsg, 5000) == pdTRUE){
        ...
        printf(p_recvMsg->pu8Buffer);
        printf(STRING_EOL);  printf(STRING_LINE);
		char *led_status_msg = strstr(p_recvMsg->pu8Buffer, "\"led\": ");
		// Converte de char '1' para int 1.
		int led_status = led_status_msg[8] - '0';
		// Toggle na flag state_button quando recebe o semaforo.
		state_button = ((xSemaphoreTake(xSemaphoreLed, 0) == pdTRUE) ^ state_button);
		// Liga o led se apenas um deles for '1' -> XOR.
		if ((led_status ^ state_button)) {
...
void init_in_out(void) {
	...
	pio_set_input(BUT_PIO, BUT_PIO_IDX_MASK, PIO_DEFAULT | PIO_DEBOUNCE);
