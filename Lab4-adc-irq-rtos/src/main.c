...
static void task_adc(void *pvParameters) {
  ...
  while (1) {
    if (xQueueReceive(xQueueADCMM, &(adcMM), 1000)) {
      printf("ADC: %d \n", adcMM);
    } else {
...
static void task_proc(void *pvParameters) {
	...
	while (1) {
		if(xQueueReceive(xQueueADC, &(adc), 0)) {
			valores[count] = (int) adc.value;
			count++;
			if (count == 10) {
				int media = media_movel(valores, 10);
				xQueueSend(xQueueADCMM, (void *)&media, 10);
				count = 0;
			}
		}
	}
...
int main(void) {
  ...
  xQueueADCMM = xQueueCreate(100, sizeof(int));
  if (xQueueADCMM == NULL)
  printf("falha em criar a queue xQueueADCMM \n");
  
	
  if (xTaskCreate(task_proc, "PROC", TASK_PROC_STACK_SIZE, NULL, TASK_PROC_STACK_PRIORITY, NULL) != pdPASS)
  printf("Failed to create task PROC task\r\n");
